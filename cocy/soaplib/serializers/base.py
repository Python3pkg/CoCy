...
def nillable_element(func):
    def wrapper(cls, element):
        if bool(element.get('{%s}nil' % cocy.soaplib.ns_xsi)): # or (element.text is None and len(element.getchildren()) == 0):
            return None
        else:
...
def string_to_xml(cls, value, tns, parent_elt, name):
    ...
    element.set('{%s}type' % cocy.soaplib.ns_xsi, cls.get_type_name_ns())
    element.text = value
...
class Base(object):
    ...
    def get_namespace_prefix(cls):
        ...
        retval = cocy.soaplib.get_namespace_prefix(ns)
    ...
    def resolve_namespace(cls, default_ns):
        if cls.__namespace__ in cocy.soaplib.const_prefmap and not cls.is_default(cls):
...
class Null(Base):
    ...
    def to_xml(cls, value, tns, parent_elt, name='retval'):
        element = etree.ElementTree.SubElement(parent_elt, "{%s}%s" % (tns,name))
        element.set('{%s}nil' % cocy.soaplib.ns_xsi, 'true')
...
class SimpleType(Base):
    ...
    def get_restriction_tag(cls, schema_entries):
        simple_type = etree.ElementTree.Element('{%s}simpleType' % cocy.soaplib.ns_xsd)
        ...
        restriction = etree.ElementTree.SubElement(simple_type,
                                            '{%s}restriction' % cocy.soaplib.ns_xsd)
        ...
        for v in cls.Attributes.values:
            enumeration = etree.ElementTree.SubElement(restriction,
                                            '{%s}enumeration' % cocy.soaplib.ns_xsd)
